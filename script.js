...
class Rgb {
    constructor(color) {
        ...
        this.g = 0;
        this.b = 0;
        this.toString = () => `rgb(${this.r}, ${this.g}, ${this.b})`;
...
class ColorConverter {
    static bpp8to24bpp(color) {
        const encodedData = (Math.floor(color.r / 32) << 5) +
...
class FileDialog {
    constructor(filter = `image/*`, multiple = true) {
        this.showDialog = () => this._fileInput.click();
        this.dispose = () => {
...
class CanvasRender {
    ...
    render(image, smoothing = false) {
        ...
        this.context.drawImage(image, 0, 0, image.naturalWidth, image.naturalHeight, 0, 0, this.width, this.height);
    }
...
function render() {
    ...
    const scaledSize = CanvasRender.calculateAspectRatioFit(image.naturalWidth, image.naturalHeight, maxSize, maxSize);
